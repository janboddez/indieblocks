( function ( blocks, element, blockEditor, components, i18n, apiFetch ) {
	...
	blocks.registerBlockType( 'indieblocks/reply-context', {
		edit: function ( props ) {
			...
			var placeholderProps = {
				icon: 'format-status',
				label: __( 'Reply Context', 'indieblocks' ),
				isColumnLayout: true,
			};
									...
									el( 'i', {},
										( ! author || 'undefined' === author )
											? element.createInterpolateElement(
												...
												sprintf( __( 'In reply to %s', 'indieblocks' ) + ( ! title.match( /[.,:!?]$/ ) ? '.' : '' ), '<a>' + ( title || url ) + '</a>' ),
												{
											...
											: element.createInterpolateElement(
												// Add a period only if the "title" doesn't already end in one of these punctuation marks.
												sprintf( __( 'In reply to %s by %s.', 'indieblocks' ), '<a>' + ( title || url ) + '</a>', '<b>' + author + '</b>' ),
												{
													a: el( 'a', { className: 'u-url', href: url } ),
					...
					: el( 'div', {
						...
						el( 'i', {},
							( ! author || 'undefined' === author )
								...
								: element.createInterpolateElement(
									// Add a period only if the "title" doesn't already end in one of these punctuation marks.
									sprintf( __( 'In reply to %s by %s.', 'indieblocks' ), '<a>' + ( title || url ) + '</a>', '<span>' + author + '</span>' ),
									{
										a: el( 'a', { className: 'u-url', href: url } ),
