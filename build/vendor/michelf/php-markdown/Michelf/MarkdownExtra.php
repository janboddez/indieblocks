...
class MarkdownExtra extends \IndieBlocks\Michelf\Markdown
{
    ...
    public function __construct()
    {
        ...
        $this->enhanced_ordered_list = \true;
        parent::__construct();
    }
    /**
    ...
    protected function setup()
    {
        ...
        foreach ($this->predef_abbr as $abbr_word => $abbr_desc) {
            ...
            $this->abbr_desciptions[$abbr_word] = \trim($abbr_desc);
        }
    }
    /**
                ...
                if ($tag[0] === "\n" || $tag[0] === " ") {
                    ...
                    if ($tag[0] === "`") {
                        ...
                        if (\preg_match('{^<(?:' . $this->block_tags_re . ')\\b}', $tag) || \preg_match('{^<(?:' . $this->context_block_tags_re . ')\\b}', $tag) && \preg_match($newline_before_re, $parsed) && \preg_match($newline_after_re, $text)) {
                            ...
                            list($block_text, $text) = $this->_hashHTMLBlocks_inHTML($tag . $text, "hashBlock", \true);
    ...
    protected function _doTable_callback($matches)
    {
        ...
        foreach ($headers as $n => $header) {
            $text .= "  <th{$attr[$n]}>" . $this->runSpanGamut(\trim($header)) . "</th>\n";
        }
        $text .= "</tr>\n";
        $text .= "</thead>\n";
        ...
        foreach ($rows as $row) {
            ...
            foreach ($row_cells as $n => $cell) {
                $text .= "  <td{$attr[$n]}>" . $this->runSpanGamut(\trim($cell)) . "</td>\n";
            }
            $text .= "</tr>\n";
        }
        $text .= "</tbody>\n";
    ...
    protected function _doDefLists_callback($matches)
    {
        ...
        $result = \trim($this->processDefListItems($list));
        $result = "<dl>\n" . $result . "\n</dl>";
    ...
    protected function _processDefListItems_callback_dt($matches)
    {
        ...
        foreach ($terms as $term) {
            $term = $this->runSpanGamut(\trim($term));
            $text .= "\n<dt>" . $term . "</dt>";
        }
        return $text . "\n";
    }
     ...
     */
    protected function formParagraphs($text, $wrap_in_p = \true)
    {
        ...
        foreach ($grafs as $key => $value) {
            $value = \trim($this->runSpanGamut($value));
        ...
        while (!empty($this->footnotes_ordered)) {
            ...
            if (!empty($this->fn_backlink_html)) {
                for ($ref_num = 1; $ref_num <= $ref_count; ++$ref_num) {
                    ...
                    $parsed_attr = $this->parseFootnotePlaceholders(\implode('', $attr), $num, $ref_num);
                    $backlink_text = $this->parseFootnotePlaceholders($this->fn_backlink_html, $num, $ref_num);
                    $ref_count_mark = $ref_num > 1 ? $ref_num : '';
                    $backlink .= " <a href=\"#fnref{$ref_count_mark}:{$note_id}\"{$parsed_attr}>{$backlink_text}</a>";
                }
                $backlink = \trim($backlink);
            }
    ...
    {
        return \str_replace(array('^^', '%%'), array($footnote_number, $reference_number), $label);
    }
    /**
    ...
    protected function doAbbreviations($text)
    {
        if ($this->abbr_word_re) {
            ...
            $text = \preg_replace_callback('{' . '(?<![\\w\\x1A])' . '(?:' . $this->abbr_word_re . ')' . '(?![\\w\\x1A])' . '}', array($this, '_doAbbreviations_callback'), $text);
        }
        return $text;
    }
    /**
