				...
				<?php if ( 'webmention' === $active_tab ) : ?>
					<table class="form-table">
						...
						<tr valign="top">
							...
							<td>
								<label><input type="checkbox" name="indieblocks_settings[cache_avatars]" value="1" <?php checked( ! empty( $this->options['cache_avatars'] ) ); ?>/> <?php esc_html_e( 'Cache avatars', 'indieblocks' ); ?></label>
								<p class="description"><?php esc_html_e( '(Experimental) Attempt to locally cache avatars. Uncheck to disable webmention avatars altogether.', 'indieblocks' ); ?></p>
								<div style="margin-inline-start: 1.25em; margin-block-start: 0.5em;">
									<label><input type="checkbox" name="indieblocks_settings[image_proxy]" value="1" <?php checked( ! empty( $this->options['image_proxy'] ) ); ?>/> <?php esc_html_e( '&ldquo;Reverse proxy&rdquo; images', 'indieblocks' ); ?></label>
									<p class="description"><?php _e( 'Serve remote images from this site&rsquo;s domain.', 'indieblocks' ); // phpcs:ignore WordPress.Security.EscapeOutput.UnsafePrintingFunction ?></p>
									<input type="text" name="indieblocks_settings[image_proxy_secret]" id="indieblocks-image-proxy-secret" style="min-width: 25%;" value="<?php echo ! empty( $this->options['image_proxy_secret'] ) ? esc_attr( $this->options['image_proxy_secret'] ) : ''; ?>" />
									<button type="button" class="button" id="indieblocks-generate-secret"><?php esc_html_e( 'Generate', 'indieblocks' ); ?></button>
									<p class="description"><?php _e( 'To work, the image proxy needs a (sufficiently random) secret, much like an autogenerated password.', 'indieblocks' ); // phpcs:ignore WordPress.Security.EscapeOutput.UnsafePrintingFunction ?></p>
								</div>
							</td>
						</tr>
						<tr valign="top">
